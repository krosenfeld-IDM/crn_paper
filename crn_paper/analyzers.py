...
class Graph():
    ...
    def draw_nodes(self, filter_func, pos, ax, **kwargs):
        inds = [i for i, n in self.graph.nodes(data=True) if filter_func(n)]
        nc = ['red' if n['hiv'] else 'lightgray' for i, n in self.graph.nodes(data=True) if i in inds]
        ec = ['green' if n['on_art'] else 'black' for i, n in self.graph.nodes(data=True) if i in inds]
        if inds:
    ...
    def plot(self, pos, edge_labels=False, ax=None):
        ...
        nx.draw_networkx_labels(self.graph, labels={i: int(a['uid']) for i, a in self.graph.nodes(data=True)},
                                font_size=8, pos=pos, ax=ax)
        if edge_labels:
            nx.draw_networkx_edge_labels(self.graph, edge_labels={(i, j): int(a['dur']) for i, j, a in self.graph.edges(data=True)},
                                         font_size=8, pos=pos, ax=ax)
        return
